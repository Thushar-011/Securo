...
from .encryption import get_encryption_key_from_session
...
class NewItemView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if action == "save":
            ...
            obj.owner = user
            try:
                key = get_encryption_key_from_session(self.request)
                obj.encrypt_sensitive_fields(key=key)
            except ValueError:
                # Fallback to deriving key if session key unavailable
...
class EditItemView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            key = get_encryption_key_from_session(request)
            item.decrypt_sensitive_fields(key=key)
        except ValueError:
            # Fallback to deriving key if session key unavailable
...
class ImportCSVView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        for row in csv_reader:
            ...
            if key:
                item.encrypt_sensitive_fields(key=key)
            else:
