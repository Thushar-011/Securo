...
class TwoFactorVerificationForm(forms.Form):
    ...
    def clean_otp(self):
        ...
        # For development help: print expected current code to console when DEBUG
        try:
            expected = totp.now()
            if settings.DEBUG:
                # print a single line with the expected code for debugging
                print(expected)
        except Exception:
            expected = None

        # Allow a ±1 step window to account for small clock skew between devices
