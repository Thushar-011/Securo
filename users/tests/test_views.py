...
class UpdateMasterPasswordViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Derive encryption key from raw password for items
        salt = base64.urlsafe_b64decode(self.user.encryption_salt)
        self.encryption_key = derive_key_from_master_password("oldpassword", salt)

        # Some items to test encryption
        self.items = []
        for _ in range(3):
            item = Item(owner=self.user, username="tester", password="testpassword", notes="Test notes")
            item.encrypt_sensitive_fields(key=self.encryption_key)
            item.save()
